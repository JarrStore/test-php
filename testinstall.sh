...
if [ "$user_token" != "$EXPECTED_TOKEN" ]; then
    echo -e "${RED}Invalid token. Exiting.${RESET}"
    exit 1
fi

# Menu for selecting action
echo -e "${CYAN}INSTAL PHPMYADMIN OTOMATIS BY FAJAR${RESET}"
...
read -p "Masukkan IP database: " DB_HOST
...
CREATE USER '${DB_NAME}'@'${DB_HOST}' IDENTIFIED BY '${DB_PASS}';
GRANT ALL PRIVILEGES ON *.* TO '${DB_NAME}'@'${DB_HOST}' WITH GRANT OPTION;
MYSQL_SCRIPT
        echo -e "${GREEN}Database user created.${RESET}"
        ;;
    ...
    2)
        ...
        echo -e "${GREEN}SSL certificate created for ${DOMAIN}.${RESET}"
        ...
        echo -e "${GREEN}Permissions and configuration set.${RESET}"
...
EOL
        ...
        echo -e "${GREEN}Nginx configuration set for phpMyAdmin.${RESET}"

        # Enable site and restart Nginx
        ln -s /etc/nginx/sites-available/phpmyadmin.conf /etc/nginx/sites-enabled/phpmyadmin.conf
        systemctl restart nginx

        echo -e "${GREEN}phpMyAdmin setup complete. You can access it at https://${DOMAIN}${RESET}"
        ;;
